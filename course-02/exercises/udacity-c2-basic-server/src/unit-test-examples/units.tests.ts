...
// @DONE try creating a new describe block for the "concat" method
...
describe('concat', () => {
  ...
  it('should return one string combining the two strings',() => {
    const result = concat('this should be ','one string');
    expect(result).to.equal('this should be one string');
  });

  it('should throw an error if either of the strings are empty', () => {
    expect(()=>{ concat('','a') }).to.throw('string empty')
  });
